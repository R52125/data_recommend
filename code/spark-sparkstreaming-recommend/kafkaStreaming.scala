...
object kafkaStreaming {
  ...
  def getStream(sc:StreamingContext) = {
    ...
    val topics = Array("test-flume-pykafka")
  ...
  def streamingCore(sc: StreamingContext): Unit = {
    ...
    stream.foreachRDD( rdd =>
      rdd.foreachPartition { part => {
        ...
        config.set("hbase.zookeeper.quorum", "bd")
        val connection = ConnectionFactory.createConnection(config)
        val table = connection.getTable(TableName.valueOf("test_movie_records"))
